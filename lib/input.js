...
function csiEvent(sequence) {
  ...
  if (csiU) {
    ...
    if ((modifiers & 4) !== 0) {
      ...
      if (codepoint === 116) return { type: "terminal" };
      if (codepoint === 110) return { type: "notes" };
...
class InputDecoder {
  ...
  readEvents() {
    ...
    while (this.buffer) {
      ...
      else if (code === 20) events.push({ type: "terminal" });
      else if (code === 14) events.push({ type: "notes" });
