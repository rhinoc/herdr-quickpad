...
function csiEvent(sequence) {
  ...
  if (mouse) {
    const rawButton = Number(mouse[1]);
    const event = {
      type: "mouse",
    ...
    if ((rawButton & 32) !== 0) event.motion = true;
    return event;
  }
  ...
  if (csiU) {
    ...
    const withShift = (type) => shift ? { type, shift: true } : { type };
    const key = String.fromCodePoint(codepoint).toLowerCase();
    ...
    if ((modifiers & 8) !== 0 && (modifiers & 4) === 0) {
      ...
      if (key === "z") return { type: shift ? "redo" : "undo" };
    ...
    if (codepoint === 57417) return withShift("left");
    if (codepoint === 57418) return withShift("right");
    if (codepoint === 57419) return withShift("up");
    if (codepoint === 57420) return withShift("down");
    if (codepoint === 57423) return withShift("home");
    if (codepoint === 57424) return withShift("end");
    ...
    if ((modifiers & 4) !== 0) {
      ...
      if (codepoint === 117) return { type: "delete-to-start" };
      if (codepoint === 107) return { type: "delete-to-end" };
      if (codepoint === 119) return { type: "delete-word" };
      if (codepoint === 100) return { type: "delete" };
      if (codepoint === 98) return withShift("left");
      if (codepoint === 102) return withShift("right");
      if (codepoint === 112) return withShift("up");
      if (codepoint === 110) return withShift("down");
      if (codepoint === 122) return { type: "undo" };
      return { type: "unknown" };
    }
...
class InputDecoder {
  ...
  readEvents() {
    ...
    while (this.buffer) {
      ...
      else if (code === 4) events.push({ type: "delete" });
      else if (code === 11) events.push({ type: "delete-to-end" });
      else if (code === 21) events.push({ type: "delete-to-start" });
      else if (code === 23) events.push({ type: "delete-word" });
      else if (code === 2) events.push({ type: "left" });
      else if (code === 6) events.push({ type: "right" });
      else if (code === 16) events.push({ type: "up" });
      else if (code === 14) events.push({ type: "down" });
      else if (code >= 32) {
